...
// App struct - now uses AppService as the underlying implementation
type App struct {
	ctx        context.Context
	appService *services.AppService

	// Keep backward compatibility for any direct field access
	config                   *Config
...
func NewApp() *App {
	// Create AppService with default config
	appService := services.NewAppService("config.json")

	// 기본 설정으로 초기화 (backward compatibility)
...
func (a *App) OnStartup(ctx context.Context) {
	...
	// Initialize AppService first
	if err := a.appService.Initialize(ctx); err != nil {
		monitoring.LogError("Failed to initialize AppService", "error", err)
		// Fall back to old initialization for backward compatibility
		a.fallbackInitialization(ctx)
		return
	}

	monitoring.LogInfo("AppService initialized successfully")
}

// fallbackInitialization provides backward compatibility if AppService fails
func (a *App) fallbackInitialization(ctx context.Context) {
	monitoring.LogInfo("Using fallback initialization for backward compatibility")
...
func LoadConfig(configPath string) (*Config, error) {
	...
	if _, err := os.Stat(configPath); os.IsNotExist(err) {
		...
		config := getDefaultConfig()
		monitoring.LogInfo("[Config] Default monitoring intervals", "cpu", config.Monitoring.IntervalSeconds, "security", config.Monitoring.SecurityCheckSeconds, "gpu", config.Monitoring.GPUInfoCacheSeconds, "registry", config.Monitoring.RegistryCacheSeconds)
		return &config, nil
	}
	...
	monitoring.LogInfo("[Config] Monitoring intervals", "cpu", config.Monitoring.IntervalSeconds, "security", config.Monitoring.SecurityCheckSeconds, "gpu", config.Monitoring.GPUInfoCacheSeconds, "registry", config.Monitoring.RegistryCacheSeconds)
	monitoring.LogInfo("[Config] Monitoring flags", "cpu", config.Monitoring.EnableCpuMonitoring, "memory", config.Monitoring.EnableMemoryMonitoring, "disk", config.Monitoring.EnableDiskMonitoring, "network", config.Monitoring.EnableNetworkMonitoring)
